/***************************************************************************************************
Procedure:          dbo.usp_ibmi_order_silver
Create Date:        2024-03-21
Author:             Tom Wolfenden
Description:        Truncate and load of order Silver
Called by:            Azure Data Factory
					Pipeline: ibmi_order_master
Affected table(s):  silver.ibmi_order_master
Usage:              EXEC dbo.usp_ibmi_order_silver

****************************************************************************************************
SUMMARY OF CHANGES
#             Date(yyyy-mm-dd)    Author              Comments
------------------- ------------------ ------------------------------------------------------------
1             2024-05-09		  Jeremy Shahan		  Converted Ship Date and Ship Time
***************************************************************************************************/

CREATE   PROCEDURE [dbo].[usp_ibmi_order_silver]
AS

SET NOCOUNT ON

DELETE FROM silver.ibmi_order

INSERT INTO silver.ibmi_order
SELECT
	  TRIM(a.ORARA)																	AS order_origin_area_code
    ...
    , TRIM(a.ORCOMC)																AS order_commodity_code
    , TRIM(a.ORCOMD)																AS order_commodity_description
    , TRIM(a.ORINIT)																AS order_creation_initials
    , CONVERT(NVARCHAR, a.ORCAC)													AS order_customer_phone_area_code
    , CONVERT(NVARCHAR, a.ORCPHN)													AS order_customer_phone_number
    , CONVERT(NVARCHAR, a.ORRAC)													AS order_consignee_phone_area_code
    , CONVERT(NVARCHAR, a.ORRPHN)													AS order_consignee_phone_number
    --, TRIM(a.ORCTIM)																Unused
    , a.ORWGT																		AS order_load_weight
    --, a.ORTWGT																	Unused
    --, TRIM(a.ORHOOK)																Unused
    --, TRIM(a.ORRACK)																Unused
    , TRIM(a.ORPLLT)																AS order_pallet_count
    , TRIM(a.OROCTY)																AS order_origin_city_code
    , TRIM(a.OROST)																	AS order_origin_state
    , a.OROBEA																		AS order_origin_bea_code -- Need clarification
    , TRIM(a.OROGU)																	AS order_origin_gu_code --Need clarification
    , TRIM(a.OROSNM)																AS order_origin_city_short_name
    , TRIM(a.ORDCTY)																AS order_destination_city_code
    , TRIM(a.ORDST)																	AS order_destination_state
    , a.ORDBEA																		AS order_destination_bea_code -- Need clarification
    , TRIM(a.ORDGU)																	AS order_destination_gu_code
    , TRIM(a.ORDSNM)																AS order_destination_city_short_name
    , a.ORMILE																		AS order_miles_billable
    --, a.ORLDMI																	Unused over the last few decades
    --, a.OREMIL																	Unused over the last few decades
    , TRIM(a.ORRST)																	AS order_load_type -- R = Reefer, D = Dry, I = Intermodal
    , TRIM(a.ORSTP)																	AS order_stop_count --Need to convert to number
    ...
    , TRIM(a.OR_DSP)																AS order_dispatch_count
    --, TRIM(a.ORDSP)																Redundancy
    , TRIM(a.ORTRLR)																AS order_preload_trailer														
    , a.ORESTR																		AS order_revenue_estimation
    , TRIM(a.ORNWPK)																AS order_new_origin_area_code --Area from possible T-Call location
    , TRIM(a.ORINAR)																AS order_destination_area_code
    , TRIM(a.ORCSH)																	AS order_bill_of_lading
    , TRIM(a.ORCNS)																	AS order_purchase_order
    , TRIM(a.ORORBY)																AS order_pick_up_code  --Used by customers for tracking purposes
    --, TRIM(a.ORLS)																Unused over the last few decades
    , TRIM(a.ORPIEC)																AS order_piece_count
    , TRIM(a.ORPORC)																AS order_collection_method_code -- P = Prepaid and C = Collection T = Third Party or Blank
    ...
    , TRIM(a.ORTM)																	AS order_lane_code
    , TRIM(a.ORSEL1)																AS order_seal_code
    --, TRIM(a.ORSEL2)																Unused
    , TRIM(a.ORSERV)																AS order_service_failure_code
    , TRIM(a.ORCMTM)																AS order_driver_commit_flag
    ...
    , TRIM(a.ORSDRR)																AS order_delivery_receipt_req 
    , TRIM(a.OREDMB)																AS order_edi_message_billing_flag
    ...
    , TRIM(a.OREDFB)																AS order_edi_billing_code
    , CASE TRIM(a.OREDIO)
		WHEN 'I' THEN 'INBOUND'
    ...
    --, a.OROPLM																	Unused
    --, a.ORDPLM																	Unused
    , TRIM(a.ORCCTY)																AS order_current_city_code
    , TRIM(a.ORCST)																	AS order_current_state
    , ORLCDT.date_key_pk															AS order_loaded_call_date
    , ORECDT.date_key_pk															AS order_empty_call_date
    --, TRIM(a.ORRATO)																Unused
    --, a.ORLDWG																	Unused
    , a.ORLGT																		AS order_trailer_length --Need clarification
    , a.ORHGT																		AS order_trailer_height --Need clarification
    --, a.ORWDT																		Unused
    , TRIM(a.ORPMTF)																AS has_permit --Blank or "M"
    , TRIM(a.ORPCOM)																AS has_permit_complete
    , a.ORLAT																		AS order_latitude
    , a.ORLONG																		AS order_longitude
    , TRIM(a.ORTEN)																	AS is_tentitive_load --Blank or "C"
    , a.ORHRS																		AS order_hours_under_dispatch --Hasn't been used in 2024
    --, a.ORMIN																		Unused
    , TRIM(a.OROZN)																	AS order_origin_zone_code
    , TRIM(a.ORORG)																	AS order_origin_region_code
    , TRIM(a.ORDZN)																	AS order_destination_zone_code
    , TRIM(a.ORDRG)																	AS order_destination_region_code
    ...
    , TRIM(a.ORCARF)																AS order_carry_over_flag --Blank or "U" or "V"
    , TRIM(a.ORUTYP)																AS order_truck_type_requirement_code
    --, TRIM(a.ORTTYP)																Unused over the last few decades															
    --, TRIM(a.ORDTYP)																Unused
    --, TRIM(a.ORHAZC)																Unused
    , TRIM(a.ORFIL)																	AS order_delivery_code
    , CAST(0 AS bit) AS is_deleted
FROM data_central_lh.dbo.ibmi_order_bronze a
