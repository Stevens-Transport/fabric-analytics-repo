...
BEGIN
    ...
    BEGIN TRY
        ...
        /*========================================================
          ...
          Latest per DIODR, DIDISP, DICONT
        ========================================================*/
        ...
        LEFT JOIN gold.dim_date DIDATE
            ON a.DIDATE = DIDATE.date_ordinal
        LEFT JOIN gold.dim_date DIETAD
            ON a.DIETAD = DIETAD.date_ordinal
        LEFT JOIN gold.dim_date DITJD
            ON a.DITJD = DITJD.date_ordinal
        WHERE a.rn = 1;

        /*========================================================
          Step 1.5: Delete target rows for load numbers in this run
        ========================================================*/
        IF OBJECT_ID('tempdb..#LoadsToDelete', 'U') IS NOT NULL
            DROP TABLE #LoadsToDelete;

        SELECT DISTINCT
            TRIM(DIODR) AS load_load_number
        INTO #LoadsToDelete
        ...
        INNER JOIN #LoadsToDelete D
            ON TGT.load_load_number = D.load_load_number;

        /*========================================================
          Step 2: Update existing records
          Normally 0 rows after delete, retained for consistency
        ========================================================*/
        UPDATE TGT
        SET
            ...
            , load_dispatch_time =
                CASE
                    WHEN SRC.DITIME LIKE '%[^0-9]%' THEN NULL
                    WHEN CONVERT(INT, SRC.DITIME) <= 2359
                         AND LEN(TRIM(SRC.DITIME)) = 4
                         AND CONVERT(
                                 INT,
                                 RIGHT(TRIM(SRC.DITIME), 2)
                             ) < 60
                    THEN CONVERT(
                             TIME(0),
                             CONCAT(
                                 LEFT(TRIM(SRC.DITIME), 2),
                                 ':',
                                 RIGHT(TRIM(SRC.DITIME), 2)
                             )
                         )
            ...
            , load_dispatch_end_time =
                CASE
                    WHEN SRC.DIETAT LIKE '%[^0-9]%' THEN NULL
                    WHEN CONVERT(INT, SRC.DIETAT) <= 2359
                         AND LEN(TRIM(SRC.DIETAT)) = 4
                         AND CONVERT(
                                 INT,
                                 RIGHT(TRIM(SRC.DIETAT), 2)
                             ) < 60
                    THEN CONVERT(
                             TIME(0),
                             CONCAT(
                                 LEFT(TRIM(SRC.DIETAT), 2),
                                 ':',
                                 RIGHT(TRIM(SRC.DIETAT), 2)
                             )
                         )
    ...
    BEGIN CATCH
        ...
        RAISERROR(
            'usp_ibmi_incr_load_silver_new failed. %s',
            16,
            1,
            @ErrMsg
        ) WITH NOWAIT;
